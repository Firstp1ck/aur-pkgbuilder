...
# Local mirror of `.github/workflows/security.yml` + `.github/workflows/lint.yml`
# (fmt, clippy, audit, deny, gitleaks). Runs everything that doesn't need
# GitHub APIs.
#
...
# ── clippy ───────────────────────────────────────────────────────────────
section "clippy"
clippy_output=$(cargo clippy --all-targets -- -D warnings 2>&1) && clippy_rc=0 || clippy_rc=$?
...
# ── cargo test ───────────────────────────────────────────────────────────
section "cargo test"
test_output=$(cargo test --bin aur-pkgbuilder 2>&1) && test_rc=0 || test_rc=$?
if [[ $test_rc -eq 0 ]]; then
  pass "cargo test --bin aur-pkgbuilder"
else
  printf "%s\n" "$test_output"
  fail "cargo test: failing tests"
fi
