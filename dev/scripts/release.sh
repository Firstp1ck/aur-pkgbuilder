...
#       Automates version bumps, docs, building, tag push (CI publishes GitHub Release), and AUR.
#
# Usage:
...
AUR_PKGBUILDER_DIR="$(realpath "${SCRIPT_DIR}/../..")"
DEV_SCRIPTS_DIR="${AUR_PKGBUILDER_DIR}/dev/scripts"
AUR_PUSH_SCRIPT="${DEV_SCRIPTS_DIR}/aur-push.sh"
UPDATE_SHA_SCRIPT="${DEV_SCRIPTS_DIR}/update-sha256sums.sh"
AUR_BIN_DIR="${AUR_PKGBUILDER_AUR_BIN_DIR:-${HOME}/aur-packages/aur-pkgbuilder-bin}"
AUR_GIT_DIR="${AUR_PKGBUILDER_AUR_GIT_DIR:-${HOME}/aur-packages/aur-pkgbuilder-git}"
# Must match matrix `asset` names in .github/workflows/release.yml (uploaded release filenames).
RELEASE_ASSET_X86_64="aur-pkgbuilder-x86_64"
RELEASE_ASSET_AARCH64="aur-pkgbuilder-aarch64"
WIKI_DIR="${AUR_PKGBUILDER_WIKI_DIR:-}"
DRY_RUN=false
...
initialize_release_report() {
  local report_dir ts
  report_dir="${AUR_PKGBUILDER_DIR}/dev/RELEASE"
  mkdir -p "${report_dir}"
  ts="$(date +%Y%m%d-%H%M%S)"
...
enable_report_mirroring() {
  ...
  if [[ "${RELEASE_REPORT_MIRRORING_ENABLED}" != true ]]; then
    ...
    echo "aur-pkgbuilder release log started: $(date -Iseconds)"
...
wait_for_release_asset() {
  ...
  local asset_url="https://github.com/Firstp1ck/aur-pkgbuilder/releases/download/${tag}/${asset_name}"
  local i
...
get_current_version() {
  sed -n 's/^version = "\(.*\)"/\1/p' "${AUR_PKGBUILDER_DIR}/Cargo.toml" | awk 'NR==1{print; exit}'
}
...
phase1_version_update() {
  ...
  if [[ "${DRY_RUN}" == true ]]; then
    ...
    sed -i "s/^version = \"${current_ver}\"/version = \"${new_ver}\"/" "${AUR_PKGBUILDER_DIR}/Cargo.toml"
    log_success "Updated Cargo.toml"
  fi

  log_step "Updating Cargo.lock"
  cd "${AUR_PKGBUILDER_DIR}"
...
phase2_documentation() {
  local new_ver="${1}"
  local old_ver="${2}"
  local release_file="${AUR_PKGBUILDER_DIR}/Release-docs/RELEASE_v${new_ver}.md"
  local announcement_file="${AUR_PKGBUILDER_DIR}/dev/ANNOUNCEMENTS/version_announcement_content.md"
  ...
  if [[ "${DRY_RUN}" == true ]]; then
    log_info "[DRY-RUN] Would run update_version_announcement.py when present"
  else
    if [[ ! -f "${announcement_file}" ]]; then
      log_warn "Announcement file not found, skipping..."
    elif [[ ! -f "${AUR_PKGBUILDER_DIR}/dev/scripts/update_version_announcement.py" ]]; then
      log_warn "dev/scripts/update_version_announcement.py not found — skipping in-repo announcement wiring"
    else
      python3 "${AUR_PKGBUILDER_DIR}/dev/scripts/update_version_announcement.py" \
        "${new_ver}" \
        "Version ${new_ver}" \
...
generate_announcement() {
  ...
  local output_file="${AUR_PKGBUILDER_DIR}/dev/ANNOUNCEMENTS/version_announcement_content.md"
...
phase3_pkgbuild_updates() {
  ...
  log_step "Update aur-pkgbuilder-bin PKGBUILD"
  if [[ "${SKIP_AUR_BIN_PROCESS}" == true ]]; then
    log_info "Skipping aur-pkgbuilder-bin PKGBUILD update."
    mark_skipped "Phase 3.1: aur-pkgbuilder-bin PKGBUILD update"
    ...
    mark_done "Phase 3.1: aur-pkgbuilder-bin PKGBUILD updated"
  fi

  log_step "Update aur-pkgbuilder-git PKGBUILD"
  if [[ "${SKIP_AUR_GIT_PROCESS}" == true ]]; then
    log_info "Skipping aur-pkgbuilder-git PKGBUILD update."
    mark_skipped "Phase 3.2: aur-pkgbuilder-git PKGBUILD update"
  elif [[ "${DRY_RUN}" == true ]]; then
    ...
    mark_done "Phase 3.2: aur-pkgbuilder-git PKGBUILD updated"
  fi
...
phase4_build_release() {
  ...
  if ! dry_run_cmd cargo test --bin aur-pkgbuilder; then
  ...
  if ! dry_run_cmd gitleaks detect --source "${AUR_PKGBUILDER_DIR}" --no-banner; then
  ...
  if [[ "${DRY_RUN}" == true ]]; then
    ...
    log_info "The Release workflow builds per-arch binaries, uploads SHA256SUMS, uses the tag message (or a short default) as the release body, and marks prerelease when major < 1."
    if [[ -f "${release_file}" ]]; then
      log_success "Optional release notes file present (for your records): ${release_file}"
    else
      log_info "No Release-docs file at ${release_file} — tag push still triggers CI; enrich the tag message if you want fuller release notes."
    fi
    log_info "Monitor: https://github.com/Firstp1ck/aur-pkgbuilder/actions (workflow: Release)"
  fi

  log_info "Skipping crates.io (this desktop application is distributed via GitHub releases and AUR, not crates.io)."
}
...
phase5_aur_update() {
  ...
  log_info "Check: https://github.com/Firstp1ck/aur-pkgbuilder/actions"
  wait_for_user "Press Enter when GitHub Action has completed..."

  if [[ "${SKIP_AUR_BIN_PROCESS}" == true ]]; then
    log_info "Skipping AUR SHA sums (aur-pkgbuilder-bin process skipped)."
  ...
  log_step "Pushing aur-pkgbuilder-bin to AUR"
  if [[ "${SKIP_AUR_BIN_PROCESS}" == true ]]; then
    log_info "Skipping aur-pkgbuilder-bin AUR push."
    mark_skipped "Phase 5.2: aur-pkgbuilder-bin AUR push"
  elif [[ "${DRY_RUN}" == true ]]; then
    ...
    log_success "Pushed aur-pkgbuilder-bin to AUR"
    mark_done "Phase 5.2: aur-pkgbuilder-bin pushed to AUR"
  fi

  log_step "Pushing aur-pkgbuilder-git to AUR"
  if [[ "${SKIP_AUR_GIT_PROCESS}" == true ]]; then
    log_info "Skipping aur-pkgbuilder-git AUR push."
    mark_skipped "Phase 5.3: aur-pkgbuilder-git AUR push"
  elif [[ "${DRY_RUN}" == true ]]; then
    ...
    "${AUR_PUSH_SCRIPT}" || { log_warn "aur-push.sh may have failed"; confirm_continue "Continue anyway?" || return 1; }
    log_success "Pushed aur-pkgbuilder-git to AUR"
    mark_done "Phase 5.3: aur-pkgbuilder-git pushed to AUR"
  fi

  log_step "Syncing PKGBUILD(s) into upstream repo"
  if [[ "${SKIP_AUR_BIN_PROCESS}" == true ]]; then
    log_info "Skipping PKGBUILD sync (aur-pkgbuilder-bin process skipped)."
    mark_skipped "Phase 5.4: PKGBUILD sync to aur-pkgbuilder repo"
  elif [[ "${SKIP_AUR_GIT_PROCESS}" == true ]]; then
    if [[ "${DRY_RUN}" == true ]]; then
      log_info "[DRY-RUN] Would copy ${AUR_BIN_DIR}/PKGBUILD to ${AUR_PKGBUILDER_DIR}/PKGBUILD-bin"
      log_info "[DRY-RUN] Would commit and push PKGBUILD-bin only"
    else
      cp "${AUR_BIN_DIR}/PKGBUILD" "${AUR_PKGBUILDER_DIR}/PKGBUILD-bin"
      log_success "Copied PKGBUILD-bin"

      cd "${AUR_PKGBUILDER_DIR}"
      git add PKGBUILD-bin
      git commit -m "Update PKGBUILD-bin for v${new_ver}" || log_warn "PKGBUILD commit failed or nothing to commit"
      git push origin || { log_error "Failed to push PKGBUILD changes"; confirm_continue "Continue anyway?" || return 1; }
      log_success "PKGBUILD-bin changes pushed to origin"
      mark_done "Phase 5.4: PKGBUILD-bin sync to aur-pkgbuilder repo"
    fi
  elif [[ "${DRY_RUN}" == true ]]; then
    log_info "[DRY-RUN] Would copy ${AUR_BIN_DIR}/PKGBUILD to ${AUR_PKGBUILDER_DIR}/PKGBUILD-bin"
    log_info "[DRY-RUN] Would copy ${AUR_GIT_DIR}/PKGBUILD to ${AUR_PKGBUILDER_DIR}/PKGBUILD-git"
    log_info "[DRY-RUN] Would commit and push PKGBUILD changes to aur-pkgbuilder repo"
  else
    cp "${AUR_BIN_DIR}/PKGBUILD" "${AUR_PKGBUILDER_DIR}/PKGBUILD-bin"
    cp "${AUR_GIT_DIR}/PKGBUILD" "${AUR_PKGBUILDER_DIR}/PKGBUILD-git"
    log_success "Copied PKGBUILD-bin and PKGBUILD-git"
    ...
    mark_done "Phase 5.4: PKGBUILD sync to aur-pkgbuilder repo"
  fi
...
ensure_release_layout_directories() {
  ...
  while true; do
    ...
    if [[ -d "${resolved}" ]] && [[ -f "${resolved}/PKGBUILD" ]]; then
      ...
      log_success "aur-pkgbuilder-bin AUR directory: ${AUR_BIN_DIR}"
      mark_done "Configured aur-pkgbuilder-bin directory"
      break
    fi
    log_warn "aur-pkgbuilder-bin path invalid or missing PKGBUILD: ${resolved}"
    printf "%bEnter path to aur-pkgbuilder-bin clone (%bs%b to skip, %bq%b to abort): %b" "${CYAN}" "${BOLD}" "${RESET}" "${BOLD}" "${RESET}" "${RESET}"
    read -r input || return 1
    case "${input}" in
      s|S)
        SKIP_AUR_BIN_PROCESS=true
        AUR_BIN_DIR=""
        log_warn "aur-pkgbuilder-bin process will be skipped for this run."
        mark_skipped "Skipped aur-pkgbuilder-bin process by user choice"
        break
        ;;
  ...
  if [[ "${SKIP_AUR_GIT_PROCESS}" == true ]]; then
    log_info "Skipping aur-pkgbuilder-git layout check (SKIP_AUR_GIT_PROCESS=true)."
  else
    while true; do
      resolved="$(expand_tilde "${AUR_GIT_DIR}")"
      if [[ -d "${resolved}" ]] && [[ -f "${resolved}/PKGBUILD" ]]; then
        AUR_GIT_DIR="$(cd "${resolved}" && pwd)"
        log_success "aur-pkgbuilder-git AUR directory: ${AUR_GIT_DIR}"
        mark_done "Configured aur-pkgbuilder-git directory"
        break
      fi
      log_warn "aur-pkgbuilder-git path invalid or missing PKGBUILD: ${resolved}"
      printf "%bEnter path to aur-pkgbuilder-git clone (%bs%b to skip, %bq%b to abort): %b" "${CYAN}" "${BOLD}" "${RESET}" "${BOLD}" "${RESET}" "${RESET}"
      read -r input || return 1
      case "${input}" in
        s|S)
          SKIP_AUR_GIT_PROCESS=true
          AUR_GIT_DIR=""
          log_warn "aur-pkgbuilder-git process will be skipped for this run."
          mark_skipped "Skipped aur-pkgbuilder-git process by user choice"
          break
          ;;
        q|Q) log_error "Aborted."; return 1 ;;
      esac
  ...
  if [[ -d "${resolved}" ]]; then
    ...
    while true; do
      printf "%bEnter path to GitHub wiki clone (%bEnter%b/%bs%b to skip wiki push): %b" "${CYAN}" "${BOLD}" "${RESET}" "${BOLD}" "${RESET}" "${RESET}"
      read -r input || return 1
...
check_prerequisites() {
  ...
  [[ -d "${AUR_PKGBUILDER_DIR}" ]] || { log_error "aur-pkgbuilder repo root not found: ${AUR_PKGBUILDER_DIR}"; return 1; }

  log_success "All prerequisites met"
}

# What: Verifies commands and paths needed for phase 5 (AUR update) only.
# Inputs: None (uses globals AUR_PKGBUILDER_DIR, scripts, START_FROM_PHASE not read).
...
update_security_md() {
  local new_ver="${1}"
  local security_file="${AUR_PKGBUILDER_DIR}/SECURITY.md"
...
update_changelog() {
  local new_ver="${1}"
  local changelog_file="${AUR_PKGBUILDER_DIR}/CHANGELOG.md"
  local release_file="${AUR_PKGBUILDER_DIR}/Release-docs/RELEASE_v${new_ver}.md"
...
EOF
  ...
  echo
  printf "%b╔════════════════════════════════════════════════════════════════════════╗%b\n" "${BOLD_CYAN}" "${RESET}"
  printf "%b║                 AUR-PKGBUILDER RELEASE AUTOMATION                       ║%b\n" "${BOLD_CYAN}" "${RESET}"
  printf "%b╚════════════════════════════════════════════════════════════════════════╝%b\n\n" "${BOLD_CYAN}" "${RESET}"
  ...
  echo "  • GitHub release: https://github.com/Firstp1ck/aur-pkgbuilder/releases"
